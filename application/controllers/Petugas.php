...
class Petugas extends CI_Controller {
    ...
    public function tambah(){
        if(isset($_POST['create'])){
            $this->PetugasModel->insert_petugas();
            redirect('petugas');
        }else{
            $data['title'] = "Tambah Data Petugas Perpustakaan | PERPUSTAKAAN";
            $this->load->view('template/header', $data);
            $this->load->view('template/sidebar');
            $this->load->view('petugas/petugas_create');
            $this->load->view('template/footer');
        }
    }
    ...
    public function ubah($id){
        if(isset($_POST['update'])){
            $this->PetugasModel->update_petugas($id);
            redirect('petugas');
        }else{
            $data['title'] = "Perbaharui Data Petugas Perpustakaan | PERPUSTAKAAN";
            $data['petugas'] = $this->PetugasModel->get_petugas_byid($id);
            $this->load->view('template/header', $data);
            $this->load->view('template/sidebar');
            $this->load->view('petugas/petugas_update', $data);
            $this->load->view('template/footer');
        }
    }
    ...
    public function hapus($id){
        if(isset($id)){
            $this->PetugasModel->delete_petugas($id);
            redirect('petugas');
        }
    }
}
